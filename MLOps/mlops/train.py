...
def evaluate(
    dataloader: DataLoader, model: nn.Module, loss_fn, device: str
) -> Tuple[float]:
    model.eval()
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        train_loss, train_acc, _, _ = evaluate(
        ...
        print(
            "Epoch: {0:d}/{1:d}. Loss (Train/Test): {2:.3f}/{3:.3f}. \
                Accuracy (Train/Test): {4:.3f}/{5:.3f}".format(
                epoch + 1,
                num_epochs,
                train_losses[-1],
                test_losses[-1],
